...
resource "kubernetes_pod" "app" {
  depends_on = [vault_jwt_auth_backend_role.jwt]
  ...
  spec {
      ...
      projected {
        sources {
          service_account_token {
            path               = "vault"
            audience           = "https://kubernetes.default.svc.cluster.local" # The Kubernetes Issuer URL
    ...
    container {
      ...
      args = [
        ...
        # login to Vault using the JWT auth method (at path k8s)
        curl -sX POST "$VAULT_ADDR/v1/auth/k8s/login" -d "$DATA" | jq .auth;
        sleep 3600;
        EOF
      ]
    }
